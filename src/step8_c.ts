...
// Fonction pour gérer la traînée
function updateTrailForShape(x, y, width, height, color, randomColor, rotation, alpha) {
    shapesTrail.push({ x, y, width, height, color, randomColor, rotation, alpha });
}

// Fonction pour dessiner la traînée
function drawTrailForShape(x, y, width, height, color, randomColor, rotation, alpha) {
    const formDrawer = new FormDrawer(ctx);
    formDrawer.drawRandomRect(x, y, width, height, color, randomColor, rotation, alpha);

}

// Fonction pour dessiner chaque objet animé et sa traînée
function drawShapes() {
    for (const shape of shapes) {
        ...
        // Dessiner la traînée pour chaque objet
        updateTrailForShape(shape.x, shape.y, shape.width, shape.height, shape.color, shape.randomColor, shape.rotation, shape.alpha);
        const formDrawer = new FormDrawer(ctx);
        formDrawer.drawRandomRect(shape.x, shape.y, shape.width, shape.height, shape.color, shape.randomColor, shape.rotation, shape.alpha);

        // Restaurer la rotation du canvas à son état d'origine
        ctx.restore();

        // Supprimer l'objet du tableau s'il sort de l'écran
        if (shape.x + shape.width < 0 || shape.x > width || shape.y + shape.height < 0 || shape.y > height) {
    ...
    for (const trailShape of shapesTrail) {
        drawTrailForShape(trailShape.x, trailShape.y, trailShape.width, trailShape.height, trailShape.color, trailShape.randomColor, trailShape.rotation, trailShape.alpha);
