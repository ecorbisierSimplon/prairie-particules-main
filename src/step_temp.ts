...
canvas.addEventListener('click', (event) => {
    ...
    for (let i = 0; i < numForm; i++) {
        ...
        if (randomNumber < 1 / 3) {
            ...
            const randomRotation = Math.random() * piArc;
            const randomRadius = Math.random() * 30 + 10;
            const randomAngle = Math.random() * piArc;
...
class ShapeDrawer extends FormDrawer {
    ...
    drawShapes() {
        ...
        for (const trailShape of shapesTrail) {
            this.drawRandomRect(trailShape.x, trailShape.y, trailShape.width, trailShape.height, trailShape.color, trailShape.randomColor, trailShape.rotation, trailShape.alpha);
...
class StarDrawer extends FormDrawer {
    ...
    drawStars() {
        ...
        this.drawRandomStar(star.x, star.y, star.size, star.color, star.spikes, star.alpha);
        ...
        for (let trailStar of starsTrail) {
            this.drawRandomStar(trailStar.x, trailStar.y, trailStar.size, trailStar.spikes, trailStar.color,trailStar.alpha);
